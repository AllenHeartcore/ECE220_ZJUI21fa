...
; ************** MAIN PROGRAM IS PART OF MP2 (SLIGHTLY EDITED) *****************

	; in MP2, distance to print is stored at x38C0

        ; strings are located at x3800 and x3840
        ; M = length of string at x3800 + 1
        ; N = length of string at x3840 + 1
	; M must be stored at x38E0
	; N must be stored at x38E1
        ; insertion cost at x3880
        ; deletion cost at x3881
        ; substitution cost at x3882
        ; table built at x4000--contains M x N elements
        ; each element is Levenshtein distance, pred offset, and 012 for pred IDS type
        ;     (pred offset is 0 for start, -3 for delete, -3M for insert,
        ;      or -3(M + N) for match/substitute; IDS type is 0 for insertion,
        ;      1 for deletion, 2 for substitution, -1 for first table entry)
	;
        JSR FIND_M_NT
        JSR INIT_WORK
        HALT

MMAINT   .BLKW #1        ; 1 + length of string 1
NMAINT   .BLKW #1        ; 1 + length of string 2


; ************** THIS PART IS MP2 (along with main program) *****************

FIND_M_NT  ; calculates and stores M and N
          ; changes only R7
        ST R0,FMN_R0T
        ST R1,FMN_R1T
        ST R7,FMN_R7T
...
MLOCT	.FILL x38E0
NLOCT	.FILL x38E1
S1_PTRT  .FILL x3800     ; string 1 starting address
S2_PTRT  .FILL x3840     ; string 2 starting address


; ************** THIS PART IS MP1 *****************

STRLENT
        ; returns length of string pointed to by R0 in R1
        ; does not modify any other registers
        ST R0,STRLEN_R0T
        ST R2,STRLEN_R2T
        AND R1,R1,#0    ; set count to 0
STRLEN_LOOPT
        LDR R2,R0,#0
        BRz STRLEN_DONET
        ADD R1,R1,#1
