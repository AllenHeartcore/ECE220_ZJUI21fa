...
analyze_code (code_stat_t* cs)
{
	...
	if (0x4000 == (cs->bits[i] & 0xF000)) {
		...
		if (0xF025 == cs->bits[i]) {
		    ...
		    cs->inst[i].next_inst = NULL;
		} else if (0xF000 == (cs->bits[i] & 0xFF00)) {
		    // a TRAP other than HALT
		    cs->inst[i].flags |= INST_FLAG_TRAP;
		}
	    }
	}
    }
...
static void
print_blocks (basic_block_t* bb)
{
    int32_t i;

    if (0 != (bb->flags & BLOCK_FLAG_PRINTED)) { return; }
    bb->flags |= BLOCK_FLAG_PRINTED;

    printf ("block %04d (from x%04X): %d instructions, %d incoming\n", 
    	    bb->uid, bb->start_addr, bb->n_inst, bb->n_incoming_arcs);
    printf ("      ");
    for (i = 0; bb->n_inst > i; i++) {
        printf ("%04X ", bb->instruct[i]->address);
    }
    if (0 != (bb->flags & INST_FLAG_HALT)) {
        printf ("\n      ENDS IN HALT");
    }
    if (0 != (bb->flags & INST_FLAG_RET)) {
        printf ("\n      ENDS IN RET");
    }
    if (0 != (bb->flags & INST_FLAG_TRAP)) {
        printf ("\n      ENDS IN TRAP");
    }
    if (NULL != bb->instruct[bb->n_inst - 1]->branch_tgt) {
        printf ("\n      ENDS IN BRANCH");
    }
    if (NULL != bb->instruct[bb->n_inst - 1]->jsr_tgt) {
        printf ("\n      ENDS IN JSR(R)");
    }
    if (0 != (bb->flags & INST_FLAG_BAD_BR_TGT)) {
        printf ("\n      ENDS WITH BRANCH OUTSIDE OF CODE");
    }
    if (0 != (bb->flags & INST_FLAG_BAD_JSR_TGT)) {
        printf ("\n      ENDS WITH JSR OUTSIDE OF CODE");
    }
    if (0 != (bb->flags & INST_FLAG_UNKNOWN_JSR_TGT)) {
        printf ("\n      ENDS WITH UNKNOWN JSRR TARGET");
    }
    if (0 != (bb->flags & INST_FLAG_LEAVES_CODE)) {
        printf ("\n      LEAVES DEFINED CODE");
    }
    if (0 != (bb->flags & BLOCK_FLAG_IN_MULTIPLE)) {
        printf ("\n      APPEARS IN MULTIPLE CODE SECTIONS");
    }
    if (NULL == bb->next_block[0]) {
        printf ("\n");
    } else if (NULL == bb->next_block[1]) {
	printf ("\n      next block: %d\n", bb->next_block[0]->uid);
	print_blocks (bb->next_block[0]);
    } else {
	printf ("\n      next blocks: %d %d\n", bb->next_block[0]->uid,
		bb->next_block[1]->uid);
	print_blocks (bb->next_block[0]);
	print_blocks (bb->next_block[1]);
    }
}
...
main (int argc, const char* const argv[])
{
    ...
    blocks = build_blocks (cs, 0, cs->inst[0].address);
    if (NULL != blocks) {
        print_blocks (blocks);
    }
	...
	for (i = 0; cs->len > i; i++) {
		...
		blocks = build_blocks (cs, i, cs->inst[i].address);
		if (NULL != blocks) {
		    print_blocks (blocks);
		}
