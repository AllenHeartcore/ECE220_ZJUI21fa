...
MAINLOOP
	...
	ADD R0,R0,#0	; check the sign
	BRz EOL		; skip ahead for EOL
	BRn NEGTOK	; skip ahead for negative tokens
...
NEGTOK 	; check for -4 (other keys)
	ADD R1,R0,#4
	BRz MAINLOOP	; ignore other keys

	; closing symbol found -- check for empty stack
	ADD R1,R6,R6
	BRnp HAVESYM

	; Oops!  Stack is empty--can't close.
	LEA R0,NOOPEN
	PUTS
	BRnzp MAINLOOP

HAVESYM ; check whether the symbol matches the last opening symbol
	LDR R1,R6,#0
	ADD R1,R1,R0
	BRz MATCHED

	; Oops!  Closing symbol didn't match opening symbol.
	LEA R0,MISMAT
	PUTS
	LD R6,STKBASE	; reset the stack
	BRnzp MAINLOOP

MATCHED	; matched: pop from stack and continue
	ADD R6,R6,#1
	BRnzp MAINLOOP

EOL	; end of line typed
	LEA R0,VALID	; prepare to print valid line message
	ADD R1,R6,R6	; check whether stack is empty
	BRz LINEOK
	LEA R0,INVALID	; stack not empty
	LD R6,STKBASE	; reset the stack
LINEOK	PUTS		; print the appropriate message
	BRnzp MAINLOOP	; back to reading tokens
